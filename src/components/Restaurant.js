...
const Restaurant = ({restaurant}) => {
    ...
    const [formattedPrice, setFormattedPrice] = useState({symbol: '$', amount: 0});

    // animation
    const fadeinAttr = useSpring({from: {opacity: 0}, to: {opacity: 1}, config: config.gentle});
    ...
    useEffect(() => {
        if (restaurant) {
            const [symbol, amount] = currencyConverter(restaurant.currency, restaurant.delivery_price);
    ...
    return (
        <animated.div className="restaurant" key={restaurant.id} style={fadeinAttr}>
            <div onClick={goToDetail}
                ...
                {isLoadedImage ? null : <BlurhashContainer blurhash={restaurant.blurhash}/>}
                <div className="overlay">
                    <p className="price">{formattedPrice.symbol}{formattedPrice.amount}</p>
                    <p ref={descEl}
